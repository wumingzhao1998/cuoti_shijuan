...
#### 3. 错题练习
- ✅ 按艾宾浩斯与练习记录（下次练习时间、掌握程度、练习次数）选题，一次一题
- ✅ 会 / 不会：会则写入下次练习时间并进入下一道；不会则更新记录并出一道类似题
- ✅ 类似题最多 2 道：类似题选会即下一道，选不会可再出一道，再不会则下一道；类似题不写入飞书
- ✅ 练习记录写入飞书多维表格中的**练习记录表**（需配置 `FEISHU_PRACTICE_TABLE_ID`）
...
1. **配置练习记录表**：在环境变量、Secrets 或 `.feishu_config.json` 中配置 `FEISHU_PRACTICE_TABLE_ID`。需在同一多维表格下新建一张表，包含字段：错题record_id、上次练习时间、掌握程度、练习次数、下次练习时间；并将应用加入该表协作者（可编辑）。

2. **选择学科和知识点**：与生成题库相同；选题范围仅限筛选后的错题。

3. **开始练习**：点击「开始练习」，系统按下次练习时间与艾宾浩斯规则选一道题展示。

4. **作答与反馈**：看过题目后选「会」或「不会」。「会」：记录掌握并排定下次复习时间，进入下一道。「不会」：记录未掌握并出一道类似题（需智谱 API）；对类似题再选会/不会，选不会可再出一道（最多 2 道类似题），再不会则进入下一道。类似题不写入飞书。

5. **练完**：当没有待复习题目时，会提示「本轮可复习的题目已练完」。
...
- `FEISHU_PRACTICE_TABLE_ID`: **错题练习**用的练习记录表的 table_id（需在同一多维表格下新建表，字段：错题record_id、上次练习时间、掌握程度、练习次数、下次练习时间）
